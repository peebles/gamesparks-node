...
WebSocketClient.prototype.connect = function(requestUrl, protocols, origin, headers) {
    ...
    req.on('response', function(response) {
        ...
        //This is the error returned when the apiKey is bad
        if(response.statusCode == 403){
        	...
        	//it's another type of error, this should be recoverable
	        self.failHandshake(
	            "Server responded with a non-101 status: " +
	            response.statusCode +
	            "\nResponse Headers Follow:\n" +
...
WebSocketClient.prototype.failSecurity = function(errorDescription) {
    if (this.socket && this.socket.writable) {
        this.socket.end();
    }
    this.emit('securityFailed', new Error(errorDescription));
};
...
WebSocketClient.prototype.failHandshake = function(errorDescription) {
    ...
    this.emit('handshakeFailed', new Error(errorDescription));
};
